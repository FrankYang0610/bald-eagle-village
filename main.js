...
import { EAGLE_A_KEYFRAMES, EAGLE_B_KEYFRAMES, EAGLE_B_START_OFFSET_SECONDS, STORY_SUBTITLE_SEGMENTS } from './animationTimeline.js';
...
function animate(currentTime) {
  ...
  if (subtitles) subtitles.update(getGlobalTimeSeconds());
...
function initSceneContent() {
  ...
  loaded.allLoaded.then(function() {
    ...
    subtitles = createSubtitles({ segments: STORY_SUBTITLE_SEGMENTS });
    ...
    var eagleB = createAnimatedBaldEagle(baldEagleModel, {
      ...
      movement: {
        controller: makeKeyframeController({
          keyframes: EAGLE_B_KEYFRAMES,
          loop: false,
          heading: 'path',
          startTimeOffsetSeconds: EAGLE_B_START_OFFSET_SECONDS // slight behind Eagle A
        })
      }
    });
